...
The system persists the full evaluation payload in the \texttt{raw\_result} column along with its hash. This structure provides a basis for \emph{output-layer} auditability: it establishes a record that can be checked for consistency between the reported score and the underlying data, contingent on consistent JSON serialization.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Automated Integrity Verification:} Development of a verification harness that actively re-computes hashes for stored records, enforcing strict JSON serialization standards to eliminate false positives caused by key reordering.
