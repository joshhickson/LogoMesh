...
Welcome to the team. **LogoMesh** is our submission for the AgentX "Green Agent" track. Our goal is to create a "Public Good" benchmark for the AI industry—a standardized, open-source metric called the **Contextual Integrity Score (CIS)**.

**The Core Problem:**
Currently, AI-generated code is evaluated on *correctness* (does it pass tests?) but not on *intent* (does it match the human's architectural and business goals?). This creates "Contextual Debt"—hidden liability where code works but is unmaintainable or insecure.
...
*Current roadmap. Note that some specific tasks here may still need pruning.*
