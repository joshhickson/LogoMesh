...
function DatabaseConfig() {
  ...
  const handleConfigChange = (field: keyof DbConfig, value: string | boolean) => {
    setDbConfig(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const saveConfig = () => {
    localStorage.setItem('database_config', JSON.stringify(dbConfig));
        ...
        {generateConnectionString() && (
          <div>
            ...
            <div className="p-2 bg-gray-100 dark:bg-gray-700 rounded font-mono text-xs break-all">
              {generateConnectionString().replace(dbConfig.password, '***')}
            </div>
          </div>
        )}
